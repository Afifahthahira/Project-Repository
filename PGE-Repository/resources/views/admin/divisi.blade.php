...
<div class="max-w-7xl mx-auto py-8 px-6">
    <div class="flex items-start justify-between mb-6">
        <div>
            <h1 class="text-3xl font-semibold text-slate-900">Divisions Management</h1>
            <p class="mt-1 text-sm text-slate-500">Manage document divisions</p>
        </div>
                ...
                <div class="flex-1">
                    <div class="flex items-center justify-between">
                        ...
                        <div class="flex items-center gap-3">
                            <button 
                                onclick="openEdit({{ $divisi->id_divisi }}, '{{ $divisi->nama_divisi }}')" 
                            ...
                            @if($divisi->dokumen_count > 0)
                                <button 
                                    disabled
                                    title="Tidak dapat dihapus karena masih memiliki {{ $divisi->dokumen_count }} dokumen"
                            ...
                            @else
                                <button 
                                    onclick="openDelete({{ $divisi->id_divisi }}, '{{ $divisi->nama_divisi }}', {{ $divisi->dokumen_count }})" 
                    ...
                    <div class="border-t mt-4 pt-3 space-y-1">
                        <div class="text-xs text-slate-400">
                            {{ $divisi->dokumen_count }} documents
                        </div>
                        <div class="text-xs text-slate-400">
                            {{ $divisi->users_count }} users
                        </div>
                    </div>
                </div>
            </div>
...
<div id="modalTambah" class="fixed inset-0 bg-black/50 backdrop-blur-sm hidden items-center justify-center z-[999]">
    <div class="bg-white w-[500px] rounded-2xl shadow-xl overflow-hidden">
        ...
        <form action="{{ route('admin.divisi.store') }}" method="POST" class="p-6">
            @csrf
            <div>
                <label class="text-sm font-medium">Nama Divisi</label>
                <input type="text" name="nama_divisi" required maxlength="150"
...
<div id="modalEdit" class="fixed inset-0 bg-black/50 backdrop-blur-sm hidden items-center justify-center z-[999]">
    <div class="bg-white w-[500px] rounded-2xl shadow-xl overflow-hidden">
        ...
        <form id="formEdit" method="POST" class="p-6">
            ...
            <div>
                <label class="text-sm font-medium">Nama Divisi</label>
                <input type="text" id="edit_nama" name="nama_divisi" required maxlength="150"
                    class="w-full mt-1 rounded-lg border-gray-300 focus:border-blue-900 focus:ring-blue-900">
            </div>
...
<div id="modalDelete" class="fixed inset-0 bg-black/50 backdrop-blur-sm hidden items-center justify-center z-[999]">
    <div class="bg-white w-[400px] rounded-2xl shadow-xl overflow-hidden">
        <div class="bg-red-600 text-white px-6 py-4">
            <h2 class="text-lg font-semibold">Hapus Divisi?</h2>
        </div>

        <form id="formDelete" method="POST" class="p-6">
            @csrf
            @method('DELETE')
            <p class="text-gray-700 mb-2">Divisi "<span id="delete_nama" class="font-semibold"></span>" akan dihapus secara permanen.</p>
...
<script>
    ...
    function openEdit(id, nama) {
        ...
        document.getElementById('formEdit').action = `/admin/divisi/${id}`;
    ...
    function openDelete(id, nama, dokumenCount) {
        ...
        document.getElementById('formDelete').action = `/admin/divisi/${id}`;
        ...
        if (dokumenCount > 0) {
            warningEl.textContent = `Divisi ini tidak dapat dihapus karena masih memiliki ${dokumenCount} dokumen.`;
